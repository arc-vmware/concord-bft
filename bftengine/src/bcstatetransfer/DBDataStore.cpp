...
void DBDataStore::serializeCheckpoint(std::ostream& os, const CheckpointDesc& desc) const {
  ...
  Serializable::serialize(os, desc.digestOfLastBlock.content(), BLOCK_DIGEST_SIZE);
  Serializable::serialize(os, desc.digestOfResPagesDescriptor.content(), BLOCK_DIGEST_SIZE);
}
...
void DBDataStore::serializeResPage(
  ...
  Serializable::serialize(os, inPageDigest.content(), BLOCK_DIGEST_SIZE);
